...
class OptionManager {
    ...
    async #createWidgetOptionsPopup(e) {
        ...
        const content = BX.create("div", {
            props: {className: "weather-yandex-popup"},
    ...
    async #addElementPopup(){
       const  data = await this.#setRequest('getUserGroups');
        ...
        arrElements.push(
            BX.create("div", {
                props: {className: "weather-yandex-popup-item",name: 'switch'},
                children: [
                    BX.create("span", {
        ...
        for (const dataKey in data) {
            ...
            arrElements.push( BX.create("div", {
                props: {className: "weather-yandex-popup-item", name: `${params.name}`},
                children: [
                    BX.create("span",
                        {text:`${params.name}`,
    ...
    async #saveOptionsHandler(forms) {
        ...
        try{
            const response =  await this.#setRequest('getOptionSave', payload);
            if (!response.success) {
